...
def fplayers(name):
    ...
    pname = name
    pcount = [0,0,0]
    with open("reddit_football.csv",encoding='UTF-8') as f2:
        for line in f2:
            words = line.lower().split()
            for i in words:
                if (i==pname):
                    pcount[0] += 1
    with open("twitter_football.csv",encoding='UTF-8') as f:
        for line in f:
            words = line.lower().split()
            for i in words:
                if (i==pname):
                        pcount[1] += 1
    with open("youtube_football.csv",encoding='UTF-8') as f:
        for line in f:
            words = line.lower().split()
            for i in words:
                if (i==pname):
...
with open("youtube_football.csv",encoding='UTF-8') as f3:
    for line in f3:
        words = line.lower().split()
        for i in words:
            if(i==word1 ):
